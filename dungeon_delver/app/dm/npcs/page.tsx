...
const styles = {
  ...
  statBlock: (species: string) => ({
    ...
    fontFamily: '"MedievalSharp", "Palatino Linotype", "Book Antiqua", Palatino, serif', color: '#e8dcc8', fontSize: '0.8rem', lineHeight: 1.6,
  } as const),
  ...
  const renderStatblock = (n: GeneratedNPC) => (
    <div style={styles.statBlock(n.species)}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
        <span style={{ fontSize: '1.1rem', fontWeight: 'bold', color: '#c9a84c' }}>{n.name}</span>
        <span style={{ fontSize: '0.65rem', color: '#5a5248' }}>{n.age} · {n.profession}</span>
      </div>
      ...
      <hr style={{ borderColor: '#3d3528', margin: '8px 0' }} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: '4px', marginBottom: '8px' }}>
        {Object.entries(n.stats).map(([s, v]) => (
          <div key={s} style={{ textAlign: 'center', background: 'rgba(0,0,0,0.2)', borderRadius: '4px', padding: '4px' }}>
            <div style={{ fontSize: '0.55rem', color: '#5a5248' }}>{statLabel(s)}</div>
            <div style={{ fontWeight: 'bold' }}>{v}</div>
            <div style={{ fontSize: '0.6rem', color: '#8a7e6a' }}>{statMod(v)}</div>
          </div>
        ))}
      </div>
      <div style={{ fontStyle: 'italic', color: '#8a7e6a', marginBottom: '4px' }}>
        {n.personality.map((t, i) => <span key={i}>"{t}"{i < n.personality.length - 1 ? ' ' : ''}</span>)}
      </div>
      <div><strong style={{ color: '#8a7e6a' }}>Ideal:</strong> {n.ideal}</div>
      <div><strong style={{ color: '#c9a84c' }}>Bond:</strong> {n.bond}</div>
      <div><strong style={{ color: '#a83232' }}>Flaw:</strong> {n.flaw}</div>
      {n.notes && <div style={{ marginTop: '6px', fontStyle: 'italic', color: '#8a7e6a' }}>{n.notes}</div>}
    </div>
  );
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={{ ...styles.panel, display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
        <button onClick={handleGenerate} style={{ padding: '10px 24px', background: '#c9a84c', border: 'none', color: '#e8dcc8', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.9rem' }}>
          ⚡ Generate NPC
        </button>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <span style={{ fontSize: '0.75rem', color: '#8a7e6a' }}>Batch:</span>
          <input type="number" min={1} max={50} value={batchCount} onChange={e => setBatchCount(Math.min(50, Math.max(1, parseInt(e.target.value) || 1)))}
            style={{ width: '50px', padding: '6px', background: '#1a1714', border: '1px solid #3d3528', borderRadius: '4px', color: '#e8dcc8', textAlign: 'center', fontSize: '0.8rem' }} />
          <button onClick={handleBatch} style={{ padding: '8px 16px', background: '#3d3528', border: 'none', color: '#e8dcc8', borderRadius: '6px', cursor: 'pointer', fontSize: '0.8rem' }}>
            Generate {batchCount}
          </button>
        </div>
        <select onChange={e => setSpeciesFilter(e.target.value)} value={speciesFilter}
          style={{ padding: '6px 10px', background: '#1a1714', color: '#e8dcc8', border: '1px solid #3d3528', borderRadius: '4px', fontSize: '0.75rem', marginLeft: 'auto' }}>
      ...
      {npc && (
        <div style={{ maxWidth: '400px' }}>
          {renderStatblock(npc)}
          <button onClick={() => handleSave(npc)} style={{ padding: '6px 14px', background: '#16a34a', border: 'none', color: '#e8dcc8', borderRadius: '4px', cursor: 'pointer', fontSize: '0.75rem', marginTop: '4px' }}>
            Save NPC
          </button>
        </div>
      )}
      ...
      {batch.length > 0 && (
        <div style={{ marginTop: '1rem' }}>
          <h3 style={{ color: '#c9a84c', margin: '0 0 8px 0', fontSize: '0.9rem' }}>Batch Results ({batch.length})</h3>
          <div style={styles.npcGrid}>
            {batch.map(n => (
              <div key={n.id}>
                {renderStatblock(n)}
                <button onClick={() => handleSave(n)} style={{ padding: '4px 10px', background: '#16a34a', border: 'none', color: '#e8dcc8', borderRadius: '4px', cursor: 'pointer', fontSize: '0.7rem', marginTop: '4px' }}>
                  Save
                </button>
              </div>
            ))}
      ...
      {savedNpcs.length > 0 && (
        <div style={{ marginTop: '2rem' }}>
          <h2 style={{ color: '#c9a84c', fontSize: '1.2rem', marginBottom: '8px' }}>Saved NPCs ({filteredSaved.length})</h2>
          <div style={styles.npcGrid}>
            {filteredSaved.map(n => (
              <div key={n.id}>
                {renderStatblock(n)}
                <button onClick={() => handleDelete(n.id)} style={{ padding: '4px 10px', background: '#a83232', border: 'none', color: '#e8dcc8', borderRadius: '4px', cursor: 'pointer', fontSize: '0.7rem', marginTop: '4px', marginRight: '6px' }}>
                  Delete
                </button>
                <button onClick={() => setNpc(n)} style={{ padding: '4px 10px', background: '#3d3528', border: 'none', color: '#e8dcc8', borderRadius: '4px', cursor: 'pointer', fontSize: '0.7rem', marginTop: '4px' }}>
                  View
                </button>
              </div>
            ))}
